...
CONFIG = {
    ...
    "checkpoint_path": str(ROOT / f"checkpoints/{DATASET_TAG}_stgcn_baseline_best.pt"),
    "out_csv": str(ROOT / f"results/{DATASET_TAG}_baseline_test_predictions.csv"),
    "batch_size": 32,
    "num_workers": 0,
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Evaluate ST-GCN Baseline on a test set.")
    ...
    print(f"\nRunning ST-GCN Baseline Evaluation...")
    ...
    model = STGCNClassifier(in_channels=ckpt["in_channels"], num_classes=2).to(device)
    ...
    plt.scatter(fpr[eer_idx], tpr[eer_idx], color="red", s=50, zorder=5, label=f"EER = {eer_val:.4f}")
    
    plt.xlim([-0.02, 1.02])
    plt.ylim([-0.02, 1.02])
    plt.xlabel("False Positive Rate")
    plt.ylabel("True Positive Rate")
    plt.title(f"ROC Curve - ST-GCN Baseline")
