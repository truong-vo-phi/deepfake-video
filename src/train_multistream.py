...
CONFIG = {
    ...
    "min_lr": 1e-6,
    "save_prefix": f"{DATASET_TAG}_stgcn_multistream",
}
...
def train_one_seed(seed: int, train_ds, val_ds, a_norm, c, v, ckpt_dir: Path):
    ...
    model = MultiStreamSTGCNClassifier(in_channels=c, num_classes=2).to(device)
...
def main():
    print(f"\nInitializing Multi-stream ST-GCN Training for dataset: {PATHS['dataset_name']}")
    device = torch.device(CONFIG["device"])
    ...
    for seed in CONFIG["seeds"]:
        seed_results.append(train_one_seed(seed, train_ds, val_ds, a_norm, c, v, ckpt_dir))
    ...
    print("Multi-seed multi-stream training completed.")
